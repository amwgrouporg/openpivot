...
function workbench({ caseData, selected, candidates, activeRun }) {
  if (!selected) return "";
  ...
  return `<div class="workbench-head"><div><span class="eyebrow">Selected entity</span><h2>${escapeHtml(selected.value)}</h2></div><button class="button button--quiet icon-button" type="button" data-action="close-workbench" aria-label="Close workbench">${icon("close")}</button></div>
    ...
    <div class="entity-notes"><span class="eyebrow">Analyst context</span><p>${escapeHtml(selected.notes || "No notes yet.")}</p></div>
...
export function renderEntities(model) {
  const { caseData, selected, candidates = [], activeRun = null, graphFilters = {} } = model;
  const statusValue = graphFilters.status ?? "active";
  const semanticLinks = caseData.links.filter((link) => statusValue === "all" || statusValue === "active" ? link.status !== "rejected" : link.status === statusValue);
  const contentHtml = `<div class="entities-view">
    ...
    <div class="entity-workspace">
      <section class="graph-card card"><div class="graph-toolbar"><span>${caseData.entities.length} entities · ${caseData.links.length} relationships</span><div><button class="button button--small icon-button" type="button" data-graph-action="out" aria-label="Zoom out">−</button><button class="button button--small icon-button" type="button" data-graph-action="in" aria-label="Zoom in">+</button></div></div><svg id="graph" role="img" aria-label="Entity relationship graph"></svg><div class="graph-empty"${caseData.entities.length ? " hidden" : ""}>Add an entity to begin the graph.</div><div class="sr-only" data-graph-semantic><h2>Graph text alternative</h2>${caseData.entities.map((entity) => `<button type="button" data-action="select-entity" data-id="${escapeHtml(entity.id)}">${escapeHtml(entity.type)}: ${escapeHtml(entity.value)}</button>`).join("")}${semanticLinks.map((link) => `<button type="button" data-action="open-relationship" data-id="${escapeHtml(link.id)}">${escapeHtml(link.status)} relationship: ${escapeHtml(link.rationale)}</button>`).join("")}</div></section>
      <section class="entity-browser card">${sectionHeader("Entities", caseData.entities.length)}${entityList(caseData, selected)}</section>
    </div>
  </div>`;
  return { contentHtml, workbenchHtml: workbench({ caseData, selected, candidates, activeRun }) };
}
