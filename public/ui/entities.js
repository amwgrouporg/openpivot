...
export function renderEntities(model) {
  ...
  const contentHtml = `<div class="entities-view">
    ...
    <div class="entity-workspace">
      <section class="graph-card card"><div class="graph-toolbar"><span>${semanticGraph.nodes.length} entities · ${semanticGraph.links.length} relationships</span><div><button class="button button--small icon-button" type="button" data-graph-action="out" aria-label="Zoom out">−</button><button class="button button--small icon-button" type="button" data-graph-action="in" aria-label="Zoom in">+</button></div></div><svg id="graph" role="img" aria-label="Entity relationship graph"></svg><div class="graph-empty"${semanticGraph.nodes.length ? " hidden" : ""}>No entities match the current filters.</div><div class="sr-only" data-graph-semantic><h2>Graph text alternative</h2>${semanticGraph.nodes.map((entity) => `<button type="button" data-action="graph-select-entity" data-id="${escapeHtml(entity.id)}">${escapeHtml(entity.type)}: ${escapeHtml(entity.value)}</button>`).join("")}${semanticGraph.links.map((link) => `<button type="button" data-action="open-relationship" data-id="${escapeHtml(link.id)}">${escapeHtml(relationshipStatusLabel(link.status))}; ${escapeHtml(relationshipTypeLabel(link.relationship_type))}: ${escapeHtml(link.rationale)}</button>`).join("")}</div></section>
