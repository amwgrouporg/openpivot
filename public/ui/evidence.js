...
export function renderEvidence({ caseData, draft = null }) {
  ...
  return `<div class="evidence-view view-enter">
    ...
    <form class="evidence-composer evidence-surface card" data-form="attach-evidence"><div class="card-body">
      <div class="evidence-form-grid"><div class="field"><label for="evidence-entity">Related entities</label><select id="evidence-entity" name="entity_ids" multiple size="${Math.min(4, Math.max(2, caseData.entities.length))}">${options}</select></div><div class="field"><label for="evidence-url">Source URL</label><input id="evidence-url" class="mono" name="url" type="url" value="${escapeHtml(draft?.url ?? "")}" placeholder="https://…" required></div><div class="field field--quote"><label for="evidence-quote">Source excerpt</label><textarea id="evidence-quote" name="quote" maxlength="4000" placeholder="Paste the exact external source text" required>${escapeHtml(draft?.quote ?? "")}</textarea></div><div class="field field--quote"><label for="evidence-relevance">Relevance to investigation</label><textarea id="evidence-relevance" name="relevance" placeholder="Explain what this excerpt supports, contradicts, or leaves unresolved">${escapeHtml(draft?.relevance ?? "")}</textarea></div></div>
